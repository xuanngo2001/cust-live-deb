...
#
# ./twin.sh $(xprop -root 32x '\t$0' _NET_ACTIVE_WINDOW | cut -f 2) up
# xprop	    -> x11-utils
# xwininfo  -> x11-utils
...
SEPARATOR=0
SCREEN_WIDTH=$(( $SCREEN_WIDTH - $SCREEN_LEFT - $SCREEN_RIGHT - $SEPARATOR ))
SCREEN_HEIGHT=$(( $SCREEN_HEIGHT - $SCREEN_TOP - $SCREEN_BOTTOM - $SEPARATOR ))

### Move window to the corresponding section of the screen.
case "${SECTION}" in
  
  top|up)
    X=$(( $SCREEN_LEFT + $TARGET_DECORATION_LEFT ))
    Y=$(( $SCREEN_TOP + $TARGET_DECORATION_TOP ))
    W=$(( $SCREEN_WIDTH - $TARGET_DECORATION_LEFT - $TARGET_DECORATION_RIGHT ))
    H=$(( ($SCREEN_HEIGHT - $TARGET_DECORATION_TOP - $TARGET_DECORATION_BOTTOM)/2 ))
    
    ;;
        
  bottom|down)
    X=$(( $SCREEN_LEFT + $TARGET_DECORATION_LEFT ))
    Y=$(( ($SCREEN_HEIGHT/2) + $TARGET_DECORATION_TOP ))
    W=$(( $SCREEN_WIDTH - $TARGET_DECORATION_LEFT - $TARGET_DECORATION_RIGHT ))
    H=$(( ($SCREEN_HEIGHT - $TARGET_DECORATION_TOP - $TARGET_DECORATION_BOTTOM)/2 ))
    ;;
  ...
  left)
    X=$TARGET_DECORATION_LEFT
    Y=$SCREEN_TOP
    W=$(( ($SCREEN_WIDTH - $TARGET_DECORATION_LEFT - $TARGET_DECORATION_RIGHT)/2 ))
    H=$SCREEN_HEIGHT
    
    ;;

  right)
    X=$(( $SCREEN_WIDTH / 2 + $TARGET_DECORATION_LEFT ))
    Y=0
    W=$(( $SCREEN_WIDTH / 2 - $TARGET_DECORATION_LEFT - $TARGET_DECORATION_RIGHT ))
    H=$SCREEN_HEIGHT  
    ;;
