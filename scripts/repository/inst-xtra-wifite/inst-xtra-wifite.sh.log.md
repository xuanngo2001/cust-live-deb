* >>>>>>>>> Running inst-xtra-wifite.sh ...
  * After this operation, 101 MB of additional disk space will be used.
  * After this operation, 6,068 kB of additional disk space will be used.
  * Install wifite.
  * Disk size = 5283484K. Space Used = 125384K.
  * The following NEW packages will be installed:
  ```bash
  libc-ares2 libgeoip1 libjs-openlayers libpcap0.8 libsmi2ldbl libssh-gcrypt-4
  libwireshark-data libwireshark8 libwiretap6 libwscodecs1 libwsutil7 tshark
  wireshark-common
  aircrack-ng iw reaver rfkill usbutils
  ```
