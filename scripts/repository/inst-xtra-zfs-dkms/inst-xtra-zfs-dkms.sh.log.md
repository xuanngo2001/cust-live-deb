* >>>>>>>>> Running inst-xtra-zfs-dkms.sh ...
  * After this operation, 353 kB of additional disk space will be used.
  * After this operation, 12.3 kB of additional disk space will be used.
  * After this operation, 44.0 MB of additional disk space will be used.
  * After this operation, 399 kB of additional disk space will be used.
  * After this operation, 3,555 kB of additional disk space will be used.
  * After this operation, 10.8 MB of additional disk space will be used.
  * After this operation, 4,078 kB of additional disk space will be used.
  * Install instructions: https://github.com/zfsonlinux/zfs/wiki/Debian
  * Install zfs-dkms: version: 0.7.11-1~bpo9+1, DKMS style package. DKMS vs KMOD, see http://zfsonlinux.org/generic-deb.html
  * Install zfs-initramfs to be able to boot from ZFS.
  * Import existing ZFS partitions: zpool import -a
  * Disk size = 3806468K. Space Used = 92668K. Runtime = 00:00:11:53.
  * The following NEW packages will be installed:
  ```bash
build-essential build-essential* dkms* dpkg-dev file
g++ g++-6 libc6-dev* libdpkg-perl libelf-dev
libnvpair1linux libstdc++-6-dev libuutil1linux libzfs2linux libzpool2linux
linux-headers-4.18.0-0.bpo.1-amd64* linux-headers-amd64 lsb-release* module-assistant spl-dkms
zfs-dkms zfs-initramfs zfsutils-linux
  ```
