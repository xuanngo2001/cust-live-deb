...
##################################################################
KERNEL_VERSION=$(cat binary/dpkg-version.log | grep linux-image | grep -vF 'linux-image-amd64 ' | sed 's/ .*//' | sed 's/linux-image-//')
CLD_LOGS_DIR="./install-log/cld${SYSTEM}_${DATE_STRING}_${KERNEL_VERSION}"
