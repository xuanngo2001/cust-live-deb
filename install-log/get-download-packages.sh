...
# echo | get line starting with 'Get:' | Exclude localhost | Exclude '/DiffIndex ' | Exclude 'InRelease [' | Exclude 'Packages [' | Exclude 'Translation-en [' | Exclude 'Release.gpg [' | Exclude 'Release ['
GET_LINE=$(cat "${INSTALL_LOG}" | grep "^Get:" | grep -vF 'http://localhost/' | grep -vF '/DiffIndex ' | grep -vF 'InRelease [' | grep -vF 'Packages [' | grep -vF 'Translation-en [' | grep -vF 'Release.gpg ['| grep -vF 'Release [')
